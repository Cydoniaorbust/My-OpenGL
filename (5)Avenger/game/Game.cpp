...
void Game::Init() {
	...
	Manager.SetTexture("sprite", "Res/awesome.png", GL_TRUE);
}
void Game::ProcessInput() {
	for (int i = 0; i < 257; i++) {//1024
		if (glfwGetKey(Env.GetWin(), i) == GLFW_PRESS)
			switch (i) {
			case GLFW_KEY_T: { if (LightAmb.x < 1) LightAmb += 0.05; } break;
			case GLFW_KEY_G: { if (LightAmb.x > 0) LightAmb -= 0.05; } break;
			case GLFW_KEY_Y: { if (LightDifDir.x < 1) LightDifDir += 0.05; } break;
			case GLFW_KEY_H: { if (LightDifDir.x > 0) LightDifDir -= 0.05; } break;
			case GLFW_KEY_U: { if (LightDifSpot.x < 1) LightDifSpot += 0.05; } break;
			case GLFW_KEY_J: { if (LightDifSpot.x > 0) LightDifSpot -= 0.05; } break;
...
uint ResManager::GetTexture(string name) {
	return Textures[name];
}
void ResManager::SetTexture(string name, CcharP file, GLboolean alpha) {
	Textures[name] = LoadTexture(file, alpha);
}
uint ResManager::LoadTexture(CcharP file, GLboolean alpha) {
	uint texture;
	int width, height, chans = 4;//3
