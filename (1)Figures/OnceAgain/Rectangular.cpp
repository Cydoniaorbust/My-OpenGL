...
istream &operator >> (istream &stream, Rectangular &obj) {
...
ostream &operator << (ostream &stream, Rectangular &obj) {
	stream << "\n/(x: " << obj.Fig[3] << "|y: " << obj.Fig[7] << ")";
	return stream;
};
ifstream &operator >> (ifstream &stream, Rectangular &obj) {
	char delim;
	memset(obj.Fig, 0, sizeof(obj.Fig));
	stream >>
		delim >> obj.Fig[3] >>
...
ofstream &operator << (ofstream &stream, Rectangular &obj) {
